...
def main7():
    ...
    st.write("Insira um valor para fazer a previsão:")
    value = st.number_input("", min_value=0.0, max_value=10.0, value=0.0, step=0.1)

    if st.button("Prever"):
        prediction = make_prediction(model, value)
        st.write(f"A previsão é: {prediction:.2f}")
